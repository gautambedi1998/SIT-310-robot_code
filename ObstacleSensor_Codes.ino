...
#define SONAR_NUM 3
#define MAX_DISTANCE 200     //Maximum distance to detect obstacles on the way of our robot.
#define PING_INTERVAL 33     //WE are going to loop the pings after 33 microseconds.

unsigned long pingTimer[SONAR_NUM]; // Holds the times when the next ping should happen for each sensor.
unsigned int cm[SONAR_NUM];         // Where the ping distances are stored.
uint8_t currentSensor = 0;          // so this code will keeps track of which sensor is active.

unsigned long _timerStart = 0;

int LOOPING = 40; //looping for every 40 milliseconds.

uint8_t oldSensorReading[3];    //Store last valid value of the sensors.

/*
  These will store raw sensor's value
*/
uint8_t leftSensor;
...
/*
   So here we will Store filtered sensor's value
*/
...
//here we are going to create Kalman object filters.
SimpleKalmanFilter KF_Left(2, 2, 0.01);
SimpleKalmanFilter KF_Center(2, 2, 0.01);
SimpleKalmanFilter KF_Right(2, 2, 0.01);

ros::NodeHandle nh; // this code will start the node on the Arduino board


/*
   When a new ping is received, add the sensor distance to an array.
*/
void echoCheck() {
...
/*
   this code will return the last valid value from the sensor
*/
void oneSensorCycle() {
...
/*
   Filter the readings of the sensor. Store the readings
   different than 0, and if the sensor returns 0, we return
   the last valid output. In this way, filter the false
...
/*
   Check if the time passed and return true
*/
